...
def processSeq(strfasta):
    ...
    matchpath = "c:/pyScripts/" + fn + "_Match.fasta"
    otherpath = "c:/pyScripts/" + fn + "_Other.fasta"
    io = "c:/pyScripts/" + fn + "_IO.txt"
    ...
    if len(seqlocus) > 1:
        ...
        hc1 = open(io,"a")
        if readDir == "FWD":
            hc1.write(userID, str(seqHeader), str(seqlocus), str(allele), readDir, str(newseq), "", str(lseq), str(rseq))
        else:
            hc1.write( userID, str(seqHeader), str(seqlocus), str(allele), readDir, str(newseq), str(newseq.reverse_complement()), str(lseq), str(rseq))
        hc1.close()
        hc = open(matchpath,"a")
