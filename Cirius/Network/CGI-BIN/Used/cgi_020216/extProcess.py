...
def processSeq(strfasta):
    ...
    cursor1.execute("SELECT species, allele, markers, numanal FROM tempuser WHERE userID = {0}".format(userID))
    ...
    if markers != 'NULL' and markers is not None:
            if len(sqlstr) < 3:
                sqlstr = sqlstr + "AND"
            if markers.find("|") > -1:
    ...
    if len(seqlocus) > 1:
        ...
        hc.close()
        #print "Sequence match: FWD - " + seqlocus
    else:
        hc = open(otherpath,"a")
        hc.write(">" + seqHeader + " 1:n:0:7 \n" + sequence + "\n")
        hc.close()
        #print "Sequence match: REV - " + seqlocus
    return True
...
def getFile(userID):
    ...
    return userfile, numanal

def main(userID):
    filename, numanal = getFile(userID)
