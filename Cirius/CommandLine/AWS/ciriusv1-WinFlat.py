...
def processSeq(strfasta):
    ...
    matchpath = "c:/pythonScripts/" + fn + "_Match.fasta"
    otherpath = "c:/pythonScripts/" + fn + "_Other.fasta"
    io = "c:/pythonScripts/" + fn + "_IO.txt"
    ...
    if len(seqlocus) > 1:
        ...
        print readDir, seqlocus, allele,  newseq
        inoutData = ""
        #print q.qsize()
        if readDir == "FWD":
            inoutData = str(userID) +"||"+ str(seqHeader) +"||"+  str(seqlocus) +"||"+  str(allele) +"||"+  readDir +"||"+  str(newseq) +"||"+  ""  +"||"+ str(lseq) +"||"+  str(rseq)+ "\n"
        else:
            inoutData = str(userID) +"||"+  str(seqHeader) +"||"+  str(seqlocus) +"||"+  str(allele) +"||"+  readDir +"||"+  str(newseq) +"||"+  str(newseq.reverse_complement()) +"||"+  str(lseq) +"||"+  str(rseq)+ "\n"
...
def writeResults(fd, fn):
    matchresults = "c:/pythonScripts/" + fn + "_Match.txt"
    matchresults2 = "c:/pythonScripts/" + fn + "_Match2.txt"
    io = "c:/pythonScripts/" + fn + "_IO.txt"
...
if __name__ == '__main__':
    ...
    try:
        ...
        # put the fastA file into temporary database
            ...
            matchpath = "c:/pythonScripts/Results/" + fn + st + "_Match.fasta"
            otherpath = "c:/pythonScripts/Results/" + fn + st +"_Other.fasta"
            io = "c:/pythonScripts/Results/" + fn + "_IO.text"
            hc = open(matchpath,"w")
            hc.close()
            hc = open(otherpath,"w")
            hc.close()
