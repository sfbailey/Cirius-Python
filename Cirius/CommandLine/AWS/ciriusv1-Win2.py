...
def processSeq(strfasta):
    ...
    matchpath = "c:/pythonScripts/Results/" + fn + "_" + st + "_Match.fasta"
    otherpath = "c:/pythonScripts/Results/" + fn + "_" + st +"_Other.fasta"
    ...
    if len(seqlocus) > 1:
        ...
        cursor1 = Con1.cursor()
        if readDir == "FWD":
                    #qInsert = "INSERT INTO temptbl (header, locus, allele, readdir, seq) " +\
                    #  "VALUES ( '{}','{}', '{}', '{}', '{}')".format(str(seqHeader), str(seqlocus), str(allele), readDir, str(newseq))
                    qInsert = "INSERT INTO tempcontent (userID, header, locus, allele, readdir, seq) " +\
                        "VALUES ( {},'{}', '{}', '{}', '{}', '{}')".format(userID, str(seqHeader), str(seqlocus), str(allele), readDir, str(newseq))

        else:
                    #qInsert = "INSERT INTO temptbl (header, locus, allele, readDir, seq, seq_rev) " +\
                    #   "VALUES ( '{}','{}', '{}', '{}', '{}', '{}')".format(str(seqHeader), str(seqlocus), str(allele), readDir, str(newseq), str(newseq.reverse_complement()))
                    qInsert = "INSERT INTO tempcontent (userID, header, locus, allele, readdir, seq, seq_rev) " +\
                        "VALUES ( {},'{}', '{}', '{}', '{}', '{}', '{}')".format(userID, str(seqHeader), str(seqlocus), str(allele), readDir, str(newseq), str(newseq.reverse_complement()))
        print qInsert
...
def writeResults(st, fn):
    matchresults = "c:/pythonScripts/Results/" + fn + "_" + st + "_Match.txt"
    matchresults2 = "c:/pythonScripts/Results/" + fn + "_" + st + "_Match2.txt"
    ...
    try:
        ...
        # put the fastA file into temporary database
            ...
            for fasta in SeqIO.parse(fasta_handle,"fasta"):
                if cRecords < 100:
                    #print "sending: " + str(cRecords) + "\n"
                    #p = Process(target=processSeq, args=(q,fasta,))
                    passfasta = str(userID) + "||" + st + "||" + fn + "||" + str(fasta.id) + "||" + str(fasta.seq.lower())

                    p = pool.imap_unordered(processSeq, (passfasta,))
                    #processPull.append(p)
                    #p.start()
                    #p.join()
